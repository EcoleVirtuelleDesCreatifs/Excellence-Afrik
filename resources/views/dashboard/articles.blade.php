...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h4 mb-1">Gestion des Articles</h2>
        <p class="text-muted mb-0">Créez, modifiez et gérez vos articles</p>
    </div>
...
function publishSelected() {
    const selected = document.querySelectorAll('.article-checkbox:checked').length;
    alert(`${selected} article(s) publié(s)`);
}

function draftSelected() {
    const selected = document.querySelectorAll('.article-checkbox:checked').length;
    alert(`${selected} article(s) mis en brouillon`);
}

function deleteSelected() {
    const selected = document.querySelectorAll('.article-checkbox:checked').length;
    if (confirm(`Êtes-vous sûr de vouloir supprimer ${selected} article(s) ?`)) {
        alert(`${selected} article(s) supprimé(s)`);
    }
}
</script>
@endpush
